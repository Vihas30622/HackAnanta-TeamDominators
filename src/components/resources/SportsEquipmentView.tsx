...
import { Trophy, Circle, AlertCircle, Dumbbell } from 'lucide-react';
...
export interface SportsEquipment {
    ...
    quantity: number;
    description?: string;
    // We can add icon mapping later, for now we can infer or use generic
}
...
const SportsEquipmentView = ({ onBack }: { onBack: () => void }) => {
    ...
    // Initial Seed Data (in case DB is empty) - Logic for Admin to seed later, 
    // but for Student View we just read.
    ...
    return (
        <div className="flex flex-col h-full bg-background min-h-screen">
            ...
            <div className="p-4 space-y-6 pb-24">
                {loading ? (
                    <div className="text-center py-10 text-muted-foreground">Loading specific equipment...</div>
                ) : equipment.length === 0 ? (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground">Available on campus</p>
                                    </div>
                                </div>
